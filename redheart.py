...
def main():
    ...
    font_center2 = pygame.font.SysFont("georgia", 44, bold=True)
    ...
    frames_per_step = 1.37
    fill_start_frame = int(outline_span * frames_per_step) + 20
    ...
    while running:
        ...
        for p in particles:
            if frame > p.delay and p.alpha < 255:
                p.alpha = min(255, p.alpha + 22 + random.randint(0, 4))
        ...
        center_start = fill_start_frame + 120
        if frame > center_start:
            ...
            center_surf = font_center.render(CENTER_TEXT, True, (255, 235, 235))
        ...
        text2_start = center_start + 50
        if frame > text2_start:
            progress2 = min(1.0, (frame - text2_start) / 60)
            alpha2 = int(255 * (1 - math.exp(-progress2 * 8)))

            pulse2 = 1.0 + 0.025 * math.sin(frame * 0.05 + 1.0)
            surf2 = font_center2.render(CENTER_TEXT_2, True, (255, 215, 215))

            w2 = int(surf2.get_width() * pulse2)
            h2 = int(surf2.get_height() * pulse2)
            if w2 > 0 and h2 > 0:
                surf2 = pygame.transform.smoothscale(surf2, (w2, h2))

            surf2.set_alpha(alpha2)

            if alpha2 > 10:
                glow2 = pygame.transform.smoothscale(
                    surf2, (int(surf2.get_width() * 1.4), int(surf2.get_height() * 1.4))
                )
                glow2.set_alpha(alpha2 // 5)
                glow2_rect = glow2.get_rect(center=(WIDTH / 2, HEIGHT / 2 + 90))
                screen.blit(glow2, glow2_rect)

            rect2 = surf2.get_rect(center=(WIDTH / 2, HEIGHT / 2 + 90))
